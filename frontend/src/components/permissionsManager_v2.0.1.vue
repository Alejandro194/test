<template>
	<div>
		<div class="flex-container">
			<div>
				<button class="button button1" @click="openModal('id01')">Add Entity</button>
			</div>
			<div v-if="entitys.length > 0">
				<button @click="openModal('m2')">Add Permissions to Entity</button>
			</div>
			<div v-if="rols.length > 0">
			...
			<form class="modal-content animate">
				...
				<div class="container">
					...
					<select name id v-model="sEntityName">
						<option v-for="entity in entitys" :key="entity._id">
					...
					<div v-if="(!isStringBlank(sEntityName))">
						...
						<div v-if="!isStringBlank(sPermName)" class="text-aling-center">
							<label
				...
				<div
					v-if="(!isStringBlank(sPermName))"
					class="container text-aling-center"
					style="background-color:#f1f1f1"
				>
					<button
						v-if="!evaluateRegularExpression(sPermName,'[a-zA-Z][a-zA-Z ]+[a-zA-Z]$')"
						type="button"
						@click="addPermitionToEntity()"
		...
		<div id="id01" class="modal" ref="entityModal">
			<form class="modal-content animate">
				...
				<div class="container">
					...
					/>
					<div
						v-if="!evaluateRegularExpression(tEntityName,'[a-zA-Z][a-zA-Z ]+[a-zA-Z]$')"
						class="text-aling-center"
					>
					...
					<div v-if="(!isStringBlank(tEntityName))">
						...
						<div class="flex-container">
							...
							/>
							<button
								v-if="(!isStringBlank(tPermName) && 
                                evaluateRegularExpression(tPermName,'[a-zA-Z][a-zA-Z ]+[a-zA-Z]$'))"
								class="addBtn"
								@click.prevent="tEntityPerms = addTempPerm(tEntityPerms,tPermName)"
							>+</button>
						</div>
						<div
							v-if="!evaluateRegularExpression(tPermName,'[a-zA-Z][a-zA-Z ]+[a-zA-Z]$')"
							class="text-aling-center"
						>
							<label
								class="visibility-hidden"
				...
				<div
					v-if="(!isStringBlank(tEntityName) && evaluateRegularExpression(tEntityName,'[a-zA-Z][a-zA-Z ]+[a-zA-Z]$') && tEntityPerms.length > 0)"
					class="container text-aling-center"
					style="background-color:#f1f1f1"
				>
					<button
						type="button"
						class="addBtn"
						@click="this.entitys = createEntity(tEntityName,tEntityPerms)"
		...
		<div>
			<div class="table-wrapper">
				<table>
					...
					<tr ref="addNewRol">
						<td v-if="flag == true" :colspan="permissions.length + 1" @click="flag = false">Add new rol</td>
					</tr>
					<tr v-if="flag == false">
...
export default {
	data() {
		return {
			...
			temp: "",
			rols: [
				{ _id: "ogdaogodiu", name: "ROL ONE", permissions: [] },
				{ _id: "fsfjgj", name: "ROL TWO", permissions: [] }
			],
			permissions: ["ENTITY ONE:pemr", "ENTITY TWO:permTwo"],
			entitys: [
				{ _id: "ouaugb", name: "ENTITY ONE", permissions: ["pemr"] },
				{ _id: "pdkgnkdagn", name: "ENTITY TWO", permissions: ["permTwo"] }
			],
			error: [],
			test1: [],
			duplicated: [],
	...
	methods: {
		...
		addNewRol(name) {
			...
			axios
				...
				.then(res =>
					this.rols.push({ _id: res.data._id, name, permissions: perms })
				)
				.catch(err => (this.error = [...this.error, err]));
			this.flag = true;
			return this.rols;
		...
		addPermitionToEntity() {
			...
			this.permissions = [
				...this.permissions,
				this.sEntityName + ":" + this.sPermName
			];
			this.sEntityName = "";
			this.sPermName = "";
		},
		*/
		...
		removePermissionToAllRols(perm) {
			...
			while (i < this.rols.length) {
				if (this.hasPermition(this.rols[i].permissions, parsePerm)) {
					this.rols[i].permissions = this.rols[i].permissions.filter(
						p => p != parsePerm
					);
		...
		removeAllPermission(rol) {
			rol.permissions = [];
			axios
				.put("http://localhost:5000/api/roles/removeAllPermissions/" + rol._id)
				.catch(err => (this.error = [...this.error, err]));
		},
		addAllArray(a1, a2) {
			let i = 0;
